/* $Id: NcbiBlastClient.cs 2776 2014-04-11 12:48:22Z hpm $
 * ======================================================================
 * 
 ...
 * 
 * See:
 * http://www.ebi.ac.uk/Tools/webservices/services/ncbi_blast_soap
 * http://www.ebi.ac.uk/Tools/webservices/tutorials/csharp
 * ====================================================================== */

using ServiceConnector.BlastConnector;
using System;
using System.IO;
...
namespace ServiceConnector
{
	...
	/// </summary>
	public class NcbiBlastClient : ServiceConnector.AbstractWsClient
	{
		/// <summary>Webservice proxy object</summary>
		public ServiceConnector.BlastConnector.JDispatcherService SrvProxy
		{
			get { return srvProxy; }
			set { srvProxy = value; }
		}
		private ServiceConnector.BlastConnector.JDispatcherService srvProxy = null;
		...
		protected override void ServiceProxyConnect()
		{
			...
			if (SrvProxy == null) {
				SrvProxy = new ServiceConnector.BlastConnector.JDispatcherServiceClient("JDispatcherServiceHttpPort1");
      ...
      status = SrvProxy.getStatus(new getStatusRequest(jobId))?.status;
		...
		public override void GetResults(string jobId, string outformat, string outFileBase)
		{
			...
			{ // Specified data type
                ...
                OutFile = tmpOutFile + "." + selResultType.identifier + "." + selResultType.fileSuffix;
            }
			else
			{ // Data types available
				...
				foreach (wsResultType resultType in resultTypes)
				{
                    ...
                    OutFile = tmpOutFile + "." + resultType.identifier + "." + resultType.fileSuffix;
                }
			}
